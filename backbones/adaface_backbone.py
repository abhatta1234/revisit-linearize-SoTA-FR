...
class IResNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Initialize precomputed_attention
        print("Moving the precomputed attention to the Local Rank == ",local_rank)
        self.precomputed_attention = self.gaussian_kernel(7,3).to(f"cuda:{local_rank}")
    ...
    def forward(self, x):
        ...
        # study the size of the output tensor here...
        norm = torch.norm(x, 2, 1, True)
        clamped_norm = torch.clamp(norm, min=0.001,max=100)
        output = torch.div(x, clamped_norm)
